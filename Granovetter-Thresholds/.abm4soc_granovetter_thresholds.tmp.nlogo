...
to setup
  ...
  ask turtles [
    if (count link-neighbors )* (threshold / 100) <= 1 [
      set color green
    ]
  ]

  if add-seed = true [
    ask n-of n-seeds turtles  [
      set riot? true
      set color red
    ]
...
to do-layout
  repeat 5000 [layout-spring turtles links 0.2 4 0.5]
  display
end

to go

  let prop-rioting-total-old count turtles with [riot? = true] / count turtles
  ask turtles [
    let prop-rioting 0
    if count link-neighbors > 0 [ ;; to avoid division by zero for agents with zero neighbors. In that case assume no rioting neighbors
      set prop-rioting 100 * count link-neighbors with [riot? = true] / count link-neighbors
    ]
  ...
  let prop-rioting-total-new count turtles with [riot? = true] / count turtles
  if (prop-rioting-total-new = prop-rioting-total-old) [stop]
  set time time + 1
  tick
end
...
to create-agents
  ...
  crt number-of-agents [
    ...
    if threshold-distribution = "normal" [set threshold (random-normal mean-threshold sd-threshold)]
    if threshold-distribution = "constant" [set threshold (constant-threshold)]
...
531
Proportion of rioting agents
count turtles with [riot? = true] / count turtles
17
1
15
...
700
611
Legend:\nRed = rioting\nGreen = not rioting, but vulnerable\nBlue = not rioting, not vulnerable
11
0.0
1
...
This code is based on a Netlogo model originally by Giangiacomo Bravo, to be found at https://modelingcommons.org/browse/one_model/4784#model_tabs_browse_info.

Extended by Rense Corten, October 2016:
* Added monitor for proportion rioting
* Added monitor for time
* Added stopping rule for running Go in "forever" mode

Version 3 by RC, Oct 2022:
* Added support for simulating Watts (2002) model:
	* Added option for constant thresholds
	* Added option for one seed (turtle 1) to get the process started
	* Added visualization of vulnarable actors in Watts' sense


Version 4 by RC, Aug 2023:
* Added support for spring layout of the network
* Added simple legend for colors

Verion 5 by RC, Oct 2023:
* Added switch for automatic layout, implemented in create-agents

Verion 5.1 by RC, Oct 2023:
* Added check to avoid division by zero for agents without neighbors in ask turtles
