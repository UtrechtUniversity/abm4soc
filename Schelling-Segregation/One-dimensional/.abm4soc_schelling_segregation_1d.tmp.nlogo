;; VERSION OF SCHELLING'S MODEL WITH ONE-DIMENSIONAL WORLD, NEIGHBORS ON FOUR SURROUNDING PATCHES (DISTANCE 2)

globals [
  percent-similar  ;; on the average, what percent of a turtle's neighbors
                   ;; are the same color as that turtle?
  percent-unhappy  ;; what percent of the turtles are unhappy?
  perc-unhappy-yellow ;; what percent of the yellow turtles are unhappy?
...
;;change rule descriptions from chooser to codes for convenience
  if rule-yellow = "1: any mixture"[set rule-code-yellow 1]
  if rule-yellow = "2: majority of blues"[set rule-code-yellow 2]
  if rule-yellow = "3: yellows not minority"[set rule-code-yellow 3]
  if rule-yellow = "4: yellows are majority"[set rule-code-yellow 4]
  if rule-yellow = "5: no blues"[set rule-code-yellow 5]
  if rule-yellow = "6: full of yellows"[set rule-code-yellow 6]
  if rule-yellow = "7: egual numbers of yellows and blues"[set rule-code-yellow 7]
  if rule-yellow = "8: percentages"[set rule-code-yellow 8]

  if rule-blue = "1: any mixture"[set rule-code-blue 1]
  if rule-blue = "2: majority of yellows"[set rule-code-blue 2]
  if rule-blue = "3: blues not minority"[set rule-code-blue 3]
  if rule-blue = "4: blues are majority"[set rule-code-blue 4]
  if rule-blue = "5: no yellows"[set rule-code-blue 5]
  if rule-blue = "6: full of blues"[set rule-code-blue 6]
  if rule-blue = "7: egual numbers of yellows and blues"[set rule-code-blue 7]
  ...
  if n-blue > (number - 1) [
    set n-blue (number - 1) ; .... and at least one yellow
    ]
...
to CheckSpaceyellow   ;procedure to check whether there is at least one place to move to for a yellow turtle
  ask patches[
    ...
    ;set blue-nearby count (blues-on neighbors) ;;TODO: TO BE UPDATED USING DISTANCE <3 & DISTANCE>1
    set blue-nearby count blues-on patch-set (patches with [distance myself < 5 and distance myself > 0])
    set all-nearby blue-nearby + yellow-nearby


      if rule-code-yellow = 1[
      ...
      if rule-code-yellow = 5[
        set ok-for-yellow (blue-nearby = 0 and (count turtles-here = 0)) ; no blues
        ]

      if rule-code-yellow = 6[
      ...
      set ok-for-yellow (yellow-nearby + 1 >= ( %-similar-wanted-by-yellow *  / 100 )) and (count turtles-here = 0)
       ]
    ]
...
to CheckSpaceBlue   ;procedure to check whether there is at least one place to move to for a blue turtle
  ask patches[
    ...
    set all-nearby blue-nearby + yellow-nearby
      ...
      if rule-code-blue = 5[
        set ok-for-blue (yellow-nearby = 0 and (count turtles-here = 0)) ; no yellows
        ]
      if rule-code-blue = 6[
...
to go
  ...
  if (place-for-yellow = 0 or perc-unhappy-yellow = 0) and (place-for-blue = 0 or perc-unhappy-blue = 0 )[stop]
...
to move-unhappy-turtles
  ask yellows with [ not happy? and turn = true] [
     if place-for-yellow > 0 [
       find-new-spot
       ]
     ]
...
to find-new-spot
  ...
  fd random-float 1
  if color = yellow[
      if any? other turtles-here or [not ok-for-yellow] of(patch-here)
...
to update-turtles
    ...
    if rule = 8[
      set happy? similar-nearby >= ( %-similar-wanted-by-yellow * total-nearby / 100)
    ]
  ]
...
to update-globals
  ...
  set perc-unhappy-yellow (count turtles with [not happy? and color = yellow]) / (count turtles with [color = yellow]) * 100
...
; Program) -- grant numbers yellow #9552950 and REC #9632612.
;
; Copyright 1998 by Uri Wilensky. All rights reserved.
;
; Permission to use, modify or yellowistribute this model is hereby granted,
; provided that both of the following requirements are followed:
; a) this copyright notice is included.
; b) this model will not be yellowistributed for profit without permission
;    from Uri Wilensky.
; Contact Uri Wilensky for appropriate licenses for yellowistribution for
; profit.
;
...
rule-yellow
rule-yellow
"1: any mixture" "2: majority of blues" "3: yellows not minority" "4: yellows are majority" "5: no blues" "6: full of yellows" "7: egual numbers of yellows and blues" "8: percentages"
2
...
rule-blue
rule-blue
"1: any mixture" "2: majority of yellows" "3: blues not minority" "4: blues are majority" "5: no yellows" "6: full of blues" "7: egual numbers of yellows and blues" "8: percentages"
2
...
This model is an extension of the following:  
...
This version by Corten, Rense (2006), Utrecht University
@#$#@#$#@
default
true
0
